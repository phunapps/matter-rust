...
pub fn emit_codecs(s: &mut String, c: &Cluster) {
    for d in &c.datatypes {
        if d.kind == "struct" {
            emit_struct_codec(s, d);
        }
    }
    for a in &c.attributes {
        emit_attr_decoder(s, a);
        if a.writable {
            emit_attr_encoder(s, a);
        }
    }
    for cmd in &c.commands {
        if cmd.direction == "request" {
            emit_command_encoder(s, cmd);
        } else {
            emit_response_decoder(s, cmd);
        }
    }
}

/// Read one TLV scalar `value` expression of the given metatype into a Rust
/// value expression, given the already-bound `Value` pattern binding `v`.
/// Returns `(match_pattern, build_expr)` for use inside a decoder arm.
fn read_scalar(metatype: &str, ty: &str, entry: Option<&str>, context: &str) -> (String, String) {
    match metatype {
        ...
        "enum" => ("Value::Uint(v)".into(), {
            let rust = base_type(ty, entry);
            let backing = if rust.ends_with("16") { "u16" } else { "u8" };
            format!("{ty}::from_raw({backing}::try_from(v).map_err(|_| ClusterError::InvalidLength(\"{context}\"))?)")
        }),
        "bitmap" => ("Value::Uint(v)".into(), {
            let rust = base_type(ty, entry);
            let backing = if rust.ends_with("32") {
                "u32"
            } else if rust.ends_with("16") {
                "u16"
            } else {
                "u8"
            };
...
fn write_scalar(metatype: &str, ty: &str, entry: Option<&str>, tag: &str, expr: &str) -> String {
    match metatype {
        ...
        "enum" => format!(
            "w.put_uint({tag}, u64::from({expr}.to_raw())).expect(\"infallible: vec writer\");"
        ),
        "bitmap" => format!(
            "w.put_uint({tag}, u64::from({expr}.bits())).expect(\"infallible: vec writer\");"
        ),
        _ => format!("w.put_uint({tag}, u64::from({expr})).expect(\"infallible: vec writer\");"),
    }
}

fn emit_attr_decoder(s: &mut String, a: &Attribute) {
    // Only scalar/enum/bitmap attributes get a generated decoder in M7.3.
    // (object/array attribute decoders are M7.4 scope — none of the synthetic
    // golden fixture's *attributes* are object/array; structs are exercised
    // via command/response decoders.)
    ...
    let (pat, build) = read_scalar(&a.metatype, &a.ty, a.entry_type.as_deref(), &ctx);
...
fn emit_attr_encoder(s: &mut String, a: &Attribute) {
    if a.metatype == "object" || a.metatype == "array" {
        return;
    }
...
fn emit_command_encoder(s: &mut String, cmd: &CommandDef) {
    ...
    for f in &cmd.fields {
        emit_field_write(s, f, &snake(&f.name));
    }
    line!(
        s,
...
fn emit_field_write(s: &mut String, f: &FieldDef, var: &str) {
    ...
    // fields inside commands are M7.4 (real clusters) scope.
    if !scalar {
        line!(
            s,
            "    // field `{}` ({}) — composite field codec is M7.4 scope.",
            f.name,
            f.metatype
        );
        return;
    }
    let inner = |expr: &str| write_scalar(&f.metatype, &f.ty, f.entry_type.as_deref(), &tag, expr);
...
fn emit_response_decoder(s: &mut String, cmd: &CommandDef) {
    ...
    emit_struct_decl_and_codec(s, &st, /*decl=*/ true);
}
...
fn emit_struct_codec(s: &mut String, d: &Datatype) {
    // Struct *decl* was already emitted by emit.rs::emit_struct; emit only the
    // codec here.
    emit_struct_decl_and_codec(s, d, /*decl=*/ false);
}
...
fn emit_struct_decl_and_codec(s: &mut String, d: &Datatype, decl: bool) {
    if decl {
...
fn emit_struct_field_read_arm(s: &mut String, f: &FieldDef) {
    let var = snake(&f.name);
    let scalar = matches!(
        f.metatype.as_str(),
        "boolean" | "integer" | "string" | "bytes" | "enum" | "bitmap"
    ...
    if !scalar {
        line!(
            s,
            "                // field `{}` ({}) — composite read is M7.4 scope.",
            f.name,
            f.metatype
        );
        return;
    ...
    let (pat, build) = read_scalar(&f.metatype, &f.ty, f.entry_type.as_deref(), &ctx);
    if f.nullable {
