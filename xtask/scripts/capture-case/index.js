...
async function captureResumptionAccepted(scenario) {
    ...
    const inputs = {
        ...
        new_resumption_id: newResumptionIdHex,
    };
...
// New resumption ID generated by the responder in Sigma2 / Sigma2_Resume.
// The Rust responder samples this from SystemRandom in production; the
// byte-parity tests inject this fixed value via
...
// The shared secret is the FULL 32-byte ECDH SharedSecret of the prior session
// (Matter Core §4.14.8; chip's SessionResumptionStorage and matter.js's
// resumption record both store the 32-byte ECDH output as the resumption IKM).
const PRIOR_RESUMPTION_ID_HEX = '11'.repeat(16);
const PRIOR_SHARED_SECRET_HEX = '22'.repeat(32);
