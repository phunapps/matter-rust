...
26.5.2, btleplug 0.12.0). Infinite hang now bounded (`84dd442e`); success still
blocked upstream. Full writeup:
`docs/superpowers/audits/2026-07-21-macos-ble-hang-investigation.md`.

Live BLE commissioning works from Linux/`BlueZ` (both C1/Wi-Fi and C2/Thread
were commissioned from the Pi against the ESP32-C6). From **macOS** it hangs at
the **first GATT op** — `discover_services`, before PASE. (The prior "reached
AddNOC then stalled" note was from a different, earlier attempt; this failure is
much earlier.)

**Two distinct findings:**

1. **The hang** is a btleplug 0.12.0 defect: its CoreBluetooth delegate handlers
   gate on `if error.is_none()` and **drop** any errored event
   (`didDiscoverDescriptorsForCharacteristic`, `didWriteValueForCharacteristic`,
   `didUpdateNotificationStateForCharacteristic`). btleplug fulfills
   `discover_services` only after every characteristic's descriptor discovery
   reports back, so a dropped descriptor-error event ⇒ the future never
   completes ⇒ infinite hang. A patched btleplug that emits the event on error
   makes discovery complete (proven).
2. **The real blocker** is that CoreBluetooth *rejects* the CHIPoBLE ops with
   **`CBError.uuidNotAllowed` (CBErrorDomain code 8)** — descriptor discovery,
   the C1 write, and the C2 subscribe all draw it. Even with events delivered,
   the C1 write returns an error, so commissioning cannot proceed. Skipping
   descriptor discovery does not help (write still fails). This corroborates the
   long-standing "macOS chip-tool BLE 0x407 GATT write fail" on this same rig:
   the write rejection appears to be a macOS/CoreBluetooth-environment problem
   affecting *all* clients, not btleplug-specific.

**Shipped:** `crates/matter-ble/src/central.rs` bounds the three unbounded awaits
(discover retry 12s×2, C1 write 12s, pre-connect disconnect 2s) ⇒ macOS fails in
~15-30s with a clear error instead of hanging. **Does not make macOS BLE
succeed.**

**Options** (see the audit doc): keep Linux-only + file an upstream btleplug
issue (recommended); or a scoped R&D task to chase `uuidNotAllowed` (compare
btleplug's write path to chip's `BleConnection.mm`, try WriteWithoutResponse /
disconnect-reconnect recovery) — open-ended, and a published crate can't carry a
git-patched btleplug so downstream benefit waits on upstream.
