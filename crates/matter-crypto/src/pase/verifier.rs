...
impl PaseVerifier {
    ...
    fn compute_pake2(
        ...
        // Split Ka (first 16 bytes) and Ke (last 16 bytes). Both are SPAKE2+
        // secrets (`ke` is the root session secret); zeroize them after their
        // last use so no copy lingers on the stack.
